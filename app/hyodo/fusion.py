"""로또 하이브리드 퓨전 (앙상블 가중 투표) — app.hyodo 독립 패키지.
2026-04-20: LSTM 두뇌 편입 (4두뇌 → 5두뇌). 가중치 lstm=2.0, total_weight 5.0 → 7.0.
2026-04-20: LLM 세트 source 필드 인식 — fallback 세트 llm_vec 오염 차단
2026-04-25 Layer 3: VECTOR_WEIGHTS를 hyodo_brain_weights DB에서 동적 로드.
2026-04-25 Layer 3.5: Top-K Greedy 1세트 강제 (LSTM top10 overlap 1.80 → 2.80/세트).
"""
import logging
import random

from app.hyodo.feedback import _load_brain_weights_from_db
from app.hyodo.filters import tier1_filter
from app.hyodo.predict_cluster import get_cluster_weights
from app.hyodo.predict_entropy import get_entropy_weights
from app.hyodo.predict_llm import _llm_predict
from app.hyodo.predict_markov import _markov_predict, get_markov_prob_vector
from app.hyodo.predict_statistical import _statistical_predict, get_statistical_prob_vector
...
def _vector_fusion_predict(
    ...
    # 주: hyodo_brain_weights는 stat/markov/llm/lstm 4두뇌만 시드.
