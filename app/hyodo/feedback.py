...
# init_lotto_db 시드와 동일 (hyodo_brain_weights 기본값; fusion fallback은 4뇌만 사용)
...
def update_brain_weights(
    ...
    *,
    ...
    hyodo_brain_weights에 stat/markov/llm/lstm/hyena만 UPDATE (INSERT 금지).
    ...
    try:
        ...
        try:
            ...
            row = conn_chk.execute(
                f"SELECT MAX(last_updated_draw) FROM hyodo_brain_weights "
...
def maybe_update_brain_weights_after_scoring(target_draw_no: int) -> dict:
    ...
    - 6뇌/lead1 예측값은 READ-ONLY (본 함수는 hyodo_brain_weights만 UPDATE)
    """
    conn = get_lotto_db()
    try:
        row = conn.execute(
...
def _load_brain_weights_from_db() -> dict[str, float]:
    ...
    try:
        ...
        try:
            rows = conn.execute(
                """
                SELECT brain_tag, current_weight FROM hyodo_brain_weights
            ...
            conn.close()
        if len(rows) < 4:
            logger.warning("hyodo_brain_weights DB 조회 실패 — 시드값 fallback")
            return seed
...
def _load_layer3_weights_5() -> dict[str, float]:
    ...
    try:
        ...
        try:
            ...
            conn.close()
        if len(rows) < 5:
            logger.warning("hyodo_brain_weights Layer3 5행 미충족 — 시드값 fallback")
            return seed
        got: dict[str, float] = {str(r[0]): float(r[1]) for r in rows}
        for bt in LAYER3_BRAIN_TAGS:
            if bt not in got:
                logger.warning("hyodo_brain_weights Layer3 조회 불완전 — 시드값 fallback")
                return seed
        return {bt: got[bt] for bt in LAYER3_BRAIN_TAGS}
    except Exception:  # noqa: BLE001
        logger.warning("hyodo_brain_weights Layer3 조회 실패 — 시드값 fallback")
        return seed
