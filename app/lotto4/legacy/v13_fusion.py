...
V9 fusion: _load_army4_weights → 단순 평균 (약함)
...
from app.lotto4.legacy.predict_stat import army4_stat_prob_vector
...
def v13_fusion_predict(training_draws: list[dict], n_sets: int = 5) -> list[dict]:
    ...
    pmfs = {
        "v13_stat": army4_stat_prob_vector(training_draws),
        "v13_markov": army4_markov_prob_vector(training_draws),
        "v13_combo": army4_combo_prob_vector(training_draws),
        "v13_lstm": army4_lstm_prob_vector(training_draws),
    }
        ...
        sets.append(
            {
                ...
                "reasoning": (
                    ...
                    f"(lstm w={weights.get('v13_lstm', 2.0):.2f})"
                ),
...
def v13_fusion_prob_vector(training_draws: list[dict]) -> dict[int, float]:
