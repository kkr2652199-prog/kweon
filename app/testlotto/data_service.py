"""로또 데이터 수집·조회·통계 서비스 — app.testlotto 독립 패키지."""
import json
import logging
import re
import time
...
def save_draw(draw: dict) -> bool:
    """회차 데이터를 lotto_testlotto.db에 저장한다 (중복이면 무시)."""
    conn = get_lotto_db()
    try:
        conn.execute(
...
def maybe_generate_army1_next_predictions(scored_draw_no: int) -> dict:
    ...
    from app.testlotto.predict_brain7 import save_brain7_predictions
...
def refresh_all_army_prediction_scores(target_draw_no: int) -> None:
    """테스트로또 1군 단독: 당첨 확정 시 채점·가중치·N+1 예측."""
    ...
    from app.testlotto.postmortem_engine import maybe_build_postmortem_after_scoring
